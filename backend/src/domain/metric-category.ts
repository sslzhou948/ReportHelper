...
const passthroughCategories = new Set([
  'custom',
  'exam',
  'imaging',
  'ultrasound',
...
function combinedText(input: MetricCategoryInput) {
  return [
    input.metricKey,
    input.metricName,
    input.originalMetricName,
    input.category,
    input.categoryCn
  ].map(compactText).join(' ').toLowerCase();
}

function categoryAliasFromText(input: MetricCategoryInput) {
  ...
  ].includes(token) || /blood\s*(routine|cell)|complete\s*blood\s*count|cbc|\u8840\u5e38\u89c4|\u8840\u6db2\u7ec6\u80de|\u8840\u7ec6\u80de|\u5168\u8840/.test(text)) {
  ...
  if ([
    ...
    'lipids',
    'lipid_profile'
  ].includes(token) || /lipid|cholesterol|hdl|ldl|\u8840\u8102|\u80c6\u56fa\u9187|\u7518\u6cb9\u4e09\u916f/.test(text)) {
    return 'blood_lipid';
  }
  if ([
    'thyroid',
    'thyroid_function',
    'thyroid_panel'
  ].includes(token) || /thyroid|ft3|ft4|tsh|\u7532\u529f|\u7532\u72b6\u817a/.test(text)) {
    return 'thyroid_function';
  }
  if (['liver', 'liver_function', 'hepatic_function'].includes(token) || /liver|hepatic|\u809d\u529f|\u8f6c\u6c28\u9176|\u80c6\u7ea2\u7d20/.test(text)) {
    return 'liver_function';
  }
  if (['kidney', 'kidney_function', 'renal_function'].includes(token) || /kidney|renal|\u80be\u529f|\u808c\u9150|\u5c3f\u9178|\u5c3f\u7d20/.test(text)) {
    return 'kidney_function';
  }
  if (['endocrine', 'endocrine_hormone', 'hormone', 'hormones'].includes(token) || /hormone|endocrine|acth|cortisol|progesterone|\u6fc0\u7d20|\u5185\u5206\u6ccc|\u5b55\u916e|\u96cc\u4e8c\u9187|\u76ae\u8d28\u9187/.test(text)) {
    return 'endocrine';
  }
  if (['tumor_marker', 'tumor_markers', 'tumour_marker'].includes(token) || /tumou?r|marker|cea|afp|ca125|ca199|psa|\u80bf\u7624\u6807\u5fd7/.test(text)) {
    return 'tumor_marker';
  }
  return '';
}

function categoryFromMetricIdentity(input: MetricCategoryInput) {
...
export function normalizeMetricCategory(input: MetricCategoryInput) {
  ...
  if (inferred) {
    return {
      category: inferred,
      categoryCn: categoryLabels[inferred]
    };
  }
  ...
  if (passthroughCategories.has(rawToken)) {
    return {
      category: rawToken,
      categoryCn: rawCategoryCn || categoryLabels[rawToken] || rawCategory || categoryLabels.other
    };
  }
  if (rawCategory) {
    return {
      category: rawToken || rawCategory,
      categoryCn: rawCategoryCn || rawCategory
    };
  }
  return {
    category: 'other',
    categoryCn: rawCategoryCn || categoryLabels.other
  };
}

export function isKnownMetricCategory(category: unknown) {
  return Object.prototype.hasOwnProperty.call(categoryLabels, normalizeToken(category));
}

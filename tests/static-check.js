...
assert.ok(recheckWxml.includes('class="todo-compose"') && recheckWxml.includes('bindconfirm="saveTodoDraft"') && recheckJs.includes('saveTodoDraft()'), 'recheck index must add custom todos through the shared compose panel');
assert.ok(!recheckWxml.includes('class="todo-row todo-draft-row"') && !recheckWxml.includes('class="mini-action"'), 'recheck index must not use the old inline mini-action todo editor');
...
assert.ok(!recheckNewJs.includes('wx.requestSubscribeMessage'), 'new recheck plan must not request reminder subscription messages while reminders are hidden');
assert.ok(!recheckNewJs.includes('subscribeAccepted: subscribe.subscribeAccepted'), 'new recheck plan must not expose reminder subscription state while reminders are hidden');
...
assert.ok(recheckNewWxml.includes('catchtap="toggleAllTodos"') && recheckNewJs.includes('toggleAllTodos()'), 'new recheck plan must expose a working select-all todo action');
...
assert.ok(!recheckDetailWxml.includes('mode="time"') && !recheckDetailWxml.includes('toggleReminder') && !recheckDetailJs.includes('updateReminderDays'), 'recheck detail must hide reminder settings until reminders are product-ready');

...
function normalizeMetricKeyToken(value) {
...
function isCustomMetricKey(token, input = {}) {
  const category = normalizeMetricKeyToken(`${compactText(input.category)} ${compactText(input.categoryCn)}`);
  return token.indexOf('manual_') === 0
    || token.indexOf('custom_') === 0
    || category.indexOf('custom') >= 0
    || compactText(input.categoryCn) === '自定义';
}

function canonicalMetricKey(input = {}, options = {}) {
...
function sameCanonicalMetricKey(left, right) {
...
module.exports = {
  canonicalMetricKey,
  normalizeMetricKeyToken,
  sameCanonicalMetricKey
};
